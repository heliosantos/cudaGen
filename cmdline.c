...
static void cmdline_parser_release(struct gengetopt_args_info *args_info)
{
	...
	free_multiple_field(args_info->blocks_given,
			    (void *)(args_info->blocks_arg),
			    &(args_info->blocks_orig));
	args_info->blocks_arg = 0;
	free_multiple_field(args_info->threads_given,
			    (void *)(args_info->threads_arg),
			    &(args_info->threads_orig));
...
int cmdline_parser_dump(FILE * outfile, struct gengetopt_args_info *args_info)
{
	...
	write_multiple_into_file(outfile, args_info->blocks_given, "blocks",
				 args_info->blocks_orig, 0);
	write_multiple_into_file(outfile, args_info->threads_given, "threads",
				 args_info->threads_orig, 0);
...
static void reset_group_what_I_want_todo(struct gengetopt_args_info *args_info)
{
			 ...
			 const char *prog_name, const char *additional_error)
{
	...
	if (args_info->modeGeometry_mode_counter
	    && check_multiple_option_occurrences(prog_name,
						 args_info->blocks_given,
						 args_info->blocks_min,
						 args_info->blocks_max,
						 "'--blocks' ('-b')"))
		error = 1;

	if (args_info->modeGeometry_mode_counter
	    && check_multiple_option_occurrences(prog_name,
						 args_info->threads_given,
						 args_info->threads_min,
						 args_info->threads_max,
						 "'--threads' ('-t')"))
		error = 1;

	if (args_info->what_I_want_todo_group_counter == 0) {
		fprintf(stderr,
			"%s: %d options of group what I want todo were given. One is required%s.\n",
			prog_name, args_info->what_I_want_todo_group_counter,
	...
	if (args_info->regular_code_given && !args_info->dir_given) {
		fprintf(stderr,
			"%s: '--regular-code' ('-r') option depends on option 'dir'%s\n",
			prog_name, (additional_error ? additional_error : ""));
		error = 1;
	}
	if (args_info->Force_given && !args_info->dir_given) {
		fprintf(stderr,
			"%s: '--Force' ('-F') option depends on option 'dir'%s\n",
			prog_name, (additional_error ? additional_error : ""));
		error = 1;
	}
	if (args_info->measure_given && !args_info->dir_given) {
		fprintf(stderr,
			"%s: '--measure' ('-m') option depends on option 'dir'%s\n",
			prog_name, (additional_error ? additional_error : ""));
		error = 1;
	}
	if (args_info->blocks_given && !args_info->dir_given) {
		fprintf(stderr,
			"%s: '--blocks' ('-b') option depends on option 'dir'%s\n",
			prog_name, (additional_error ? additional_error : ""));
		error = 1;
	}
	if (args_info->threads_given && !args_info->dir_given) {
		fprintf(stderr,
			"%s: '--threads' ('-t') option depends on option 'dir'%s\n",
			prog_name, (additional_error ? additional_error : ""));
		error = 1;
	}
	if (args_info->kernel_given && !args_info->dir_given) {
		fprintf(stderr,
			"%s: '--kernel' ('-k') option depends on option 'dir'%s\n",
			prog_name, (additional_error ? additional_error : ""));
		error = 1;
	}
	if (args_info->proto_given && !args_info->dir_given) {
		fprintf(stderr,
			"%s: '--proto' ('-p') option depends on option 'dir'%s\n",
			prog_name, (additional_error ? additional_error : ""));
		error = 1;
	}
...
static int
check_modes(int given1[], const char *options1[],
			...
			const char *additional_error)
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "hVd::rFmb:t:k::p:", long_options,
				&option_index);
		...
		case 'd':	/* Output directory. If it already exists, a new one will be created adding YYYY.MM.DD---HHhMM.SSs.  */
			...
			if (update_arg((void *)&(args_info->dir_arg),
				       &(args_info->dir_orig),
				       &(args_info->dir_given),
				       &(local_args_info.dir_given), optarg, 0,
				       "OutDir", ARG_STRING, check_ambiguity,
				       override, 0, 0, "dir", 'd',
				       additional_error))
				goto failure;
		...
		case 'r':	/* Generate simple C template without CUDA.  */
			...
			if (update_arg
			    ((void *)&(args_info->regular_code_flag), 0,
			     &(args_info->regular_code_given),
			     &(local_args_info.regular_code_given), optarg, 0,
			     0, ARG_FLAG, check_ambiguity, override, 1, 0,
			     "regular-code", 'r', additional_error))
				goto failure;
		...
		case 'F':	/* force destination directory.  */
			...
			if (update_arg
			    ((void *)&(args_info->Force_flag), 0,
			     &(args_info->Force_given),
			     &(local_args_info.Force_given), optarg, 0, 0,
			     ARG_FLAG, check_ambiguity, override, 1, 0, "Force",
			     'F', additional_error))
				goto failure;
		...
		case 'm':	/* kernel measure GPU execution time.  */
			...
			if (update_arg
			    ((void *)&(args_info->measure_flag), 0,
			     &(args_info->measure_given),
			     &(local_args_info.measure_given), optarg, 0, 0,
			     ARG_FLAG, check_ambiguity, override, 1, 0,
			     "measure", 'm', additional_error))
				goto failure;
		...
		case 'k':	/* Kernel name.  */
			...
			if (update_arg((void *)&(args_info->kernel_arg),
				       &(args_info->kernel_orig),
				       &(args_info->kernel_given),
				       &(local_args_info.kernel_given), optarg,
				       0, "kernel", ARG_STRING, check_ambiguity,
				       override, 0, 0, "kernel", 'k',
				       additional_error))
				goto failure;
		...
		case 'p':	/* kernel proto.  */
			...
			if (update_arg((void *)&(args_info->proto_arg),
				       &(args_info->proto_orig),
				       &(args_info->proto_given),
				       &(local_args_info.proto_given), optarg,
				       0, 0, ARG_STRING, check_ambiguity,
				       override, 0, 0, "proto", 'p',
				       additional_error))
				goto failure;
		...
		case 0:	/* Long option with no short option */
			...
			if (strcmp(long_options[option_index].name, "about") ==
			    0) {
				...
				if (update_arg(0,
					       ...
					       &(local_args_info.about_given),
					       optarg, 0, 0, ARG_NO,
					       check_ambiguity, override, 0, 0,
					       "about", '-', additional_error))
					goto failure;
	...
	if (args_info->what_I_want_todo_group_counter > 1) {
		fprintf(stderr,
			"%s: %d options of group what I want todo were given. One is required%s.\n",
			argv[0], args_info->what_I_want_todo_group_counter,
			(additional_error ? additional_error : ""));
		error = 1;
	}
	...
	update_multiple_arg((void *)&(args_info->blocks_arg),
			    ...
			    local_args_info.blocks_given, 0, ARG_INT,
			    blocks_list);
	update_multiple_arg((void *)&(args_info->threads_arg),
			    &(args_info->threads_orig),
			    args_info->threads_given,
			    local_args_info.threads_given, 0, ARG_INT,
			    threads_list);
	...
	if (args_info->modeGeometry_mode_counter
	    && args_info->modeKernel_mode_counter) {
		int modeGeometry_given[] =
		    { args_info->blocks_given, args_info->threads_given, -1 };
		const char *modeGeometry_desc[] =
		    { "--blocks", "--threads", 0 };
		int modeKernel_given[] =
		    { args_info->kernel_given, args_info->proto_given, -1 };
		const char *modeKernel_desc[] = { "--kernel", "--proto", 0 };
		error +=
		    check_modes(modeGeometry_given, modeGeometry_desc,
				modeKernel_given, modeKernel_desc);
	}
