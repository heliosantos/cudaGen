...
int main(int argc, char *argv[])
{
	...
	else
	{
	  ...
	  if(!hasOpt)
	  {
	    ERROR(1, "Are you sleeping?Hello!!!Where is the option dude?\n");
	  }
	  
	  if(cudaTemplate && cTemplate)
	  {
	    ERROR(2, "Only one option required: regular C template or another CUDA template\n");
	  }
	  ...
	  {
	    generateStaticTemplate(dirname, path, cudaTemplate, kernelProto);
