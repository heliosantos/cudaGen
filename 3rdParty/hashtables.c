...
void *tabela_remover(HASHTABLE_T * tabela, char *chave)
{
	...
	if (entrada != NULL &&
	    strcmp(entrada->chave, chave) == 0 && entrada->activo) {
		entrada->activo = 0;
		tabela->total_inactivos++;
		tabela->total_activos--;
...
{
	int i = hashing_string(chave) % t->tamanho, pos = -1, inicial = i, inc =
	    1;

	while (t->entradas[i] != NULL
	       && strcmp(t->entradas[i]->chave, chave) != 0) {
...
static void rehash(HASHTABLE_T * tabela)
{
	...
	for (i = 0; i < tamanho_antigo; i++) {
		...
		if (entrada != NULL && entrada->activo) {
			tabela_inserir(tabela, entrada->chave,
				       entrada->elemento);
			free(entrada->chave);
			free(entrada);
		}
...
{
	return (tabela->total_activos +
		tabela->total_inactivos) / (float)tabela->tamanho;
}
...
{
	ENTRADA_T **entradas =
	    (ENTRADA_T **) malloc(sizeof(ENTRADA_T *) * tamanho);
	int i;
	for (i = 0; i < tamanho; i++)
		entradas[i] = NULL;
	return entradas;
