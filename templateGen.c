...
void createDirectoryAndHeaderFile(int force, char *dirname, char **path)
{
	...
	if (!force && directoryExists(dirname)) {
		...
		sprintf(timestamp, "%04d.%02d.%02d---%02dh%02dm%02ds",
			t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
			t->tm_hour, t->tm_min, t->tm_sec);

		if ((*path =
		     (char *)malloc((strlen(dirname) + strlen(timestamp) + 1) *
				    sizeof(char) + 1)) == NULL)
			ERROR(5, "Can't malloc");

	} else {
		if ((*path =
		     (char *)malloc((strlen(dirname)) * sizeof(char) + 1)) ==
		    NULL)
			ERROR(5, "Can't malloc");
	}
...
void createHeaderTemplate(char *dirname, char *path)
{
	...
	if ((filename =
	     (char *)malloc((strlen(path) + strlen(dirname) + 4) *
			    sizeof(char))) == NULL)
		ERROR(5, "Can't malloc");
	...
	if ((symbol =
	     (char *)malloc((strlen(dirname) + 5) * sizeof(char))) == NULL)
		ERROR(5, "Can't malloc");
 ...
 */
void
generateStaticTemplate(char *dirname, char *path, int cudaTemplateByDefault,
		       char *kernelProto)
{
	...
	char *filename;
	if ((filename =
	     (char *)malloc((strlen(path) + strlen(dirname) + strlen(ext) + 2)
			    * sizeof(char))) == NULL)
		ERROR(5, "Can't malloc");
...
int createErrorFile(char *path)
{
	...
	if ((filename =
	     malloc((strlen(path) + strlen(error) + 2) * sizeof(char))) == NULL)
		return FALSE;
	...
	fprintf(fptr, "    {\n");
	fprintf(fptr,
		"        printf( \"[ERROR] '%s' (%s) in '%s' at line '%s'\n\",\n",
		"%s", "%d", "%s", "%d");
